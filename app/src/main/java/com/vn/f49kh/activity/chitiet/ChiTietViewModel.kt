...
class ChiTietViewModel(app: Application) : BaseF49ViewModel(app) {
    ...
    var noidung: MutableLiveData<String> = MutableLiveData()
...
//        showLoading()
    ...
    fun getChiTietThanhLy(idHopDOng: Int?) {
        showLoading()
        handleRequestServiceObject(mApiService.getChiTietThanhLy(idHopDOng)) {
            it?.let {
                listImage.value = it.listImage?.toMutableList()?.map { it.url ?: "" }?.toMutableList()
                price.value = it.giaThanhLy?.toPrice()
                nameProduct.value = it?.tenVatCamCo
